...
struct openGLObject {
    ...
    // 环境光反射、漫反射、镜面反射 控制变量
    GLuint ambientOpen = false;
    GLuint DiffuseOpen = false;
    GLuint SpecularOpen = false;
};
...
void init() {
    ...
    // 读取着色器并使用
    vshader = "lab/labShaders/lab3HomeWork/vshader.glsl";
    fshader = "lab/labShaders/lab3HomeWork/fshader.glsl";
...
void display() {
    ...
    //光源环绕转动，按R开启
    if(isRotate){
        float radius = 2.0f, lightTmpHeight = light->getTranslation().y;
        light->setTranslation(glm::vec3(radius*cos(glfwGetTime()),
                                        lightTmpHeight,
    ...
    //  判断光源的y值是否小于等于(minV+maxV)/2+0.3f，如果是，那么就不绘制阴影
    if (light->getTranslation().y > (minV + maxV) / 2.0f + 0.3f) {
        // TODO:绘制阴影，修改getShadowProjectionMatrix
        glm::mat4 shadow = light->getShadowProjectionMatrix(minV);
        shadow = shadow * modelMatrix;
        glUniform1i(mesh_object.shadowLocation, 1);
        glUniformMatrix4fv(mesh_object.modelLocation, 1, GL_FALSE, &shadow[0][0]);
        glDrawArrays(GL_TRIANGLES, 0, mesh->getPoints().size());
    }
}
...
void mainWindow_key_callback(GLFWwindow *window, int key, int scancode, int action, int mode) {
    ...
    if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) {
        ...
        mesh = new TriMesh();
        std::pair<float, float> pa = mesh->readOff_usepair("./assets/sphere.off");
        minV = pa.first, maxV = pa.second;
        init();
    } else if (key == GLFW_KEY_A && action == GLFW_PRESS) {
        std::cout << "read Pikachu.off" << std::endl;
        mesh = new TriMesh();
        std::pair<float, float> pa = mesh->readOff_usepair("./assets/Pikachu.off");
        minV = pa.first, maxV = pa.second;
        init();
    } else if (key == GLFW_KEY_W && action == GLFW_PRESS) {
        std::cout << "read Squirtle.off" << std::endl;
        mesh = new TriMesh();
        std::pair<float, float> pa = mesh->readOff_usepair("./assets/Squirtle.off");
        minV = pa.first, maxV = pa.second;
        init();
    } else if (key == GLFW_KEY_S && action == GLFW_PRESS) {
        std::cout << "read sphere_coarse.off" << std::endl;
        mesh = new TriMesh();
        std::pair<float, float> pa = mesh->readOff_usepair("./assets/sphere_coarse.off");
        minV = pa.first, maxV = pa.second;
        init();
        ...
        glUniform1i(mesh_object.ambientOpen, true);
        glUniform1i(mesh_object.DiffuseOpen, false);
        glUniform1i(mesh_object.SpecularOpen, false);
    } else if (key == GLFW_KEY_5 && action == GLFW_PRESS && mode == GLFW_MOD_SHIFT) {
        glUniform1i(mesh_object.ambientOpen, false);
        glUniform1i(mesh_object.DiffuseOpen, false);
        glUniform1i(mesh_object.SpecularOpen, false);
    } else if (key == GLFW_KEY_6 && action == GLFW_PRESS && mode == 0x0000) {
        glUniform1i(mesh_object.ambientOpen, false);
        glUniform1i(mesh_object.DiffuseOpen, true);
        glUniform1i(mesh_object.SpecularOpen, false);
        ...
        glUniform1i(mesh_object.ambientOpen, false);
        glUniform1i(mesh_object.DiffuseOpen, false);
        glUniform1i(mesh_object.SpecularOpen, true);
...
int main(int argc, char **argv) {
    ...
    GLFWwindow *mainwindow = glfwCreateWindow(600, 600, "2020274045_缪克达_实验3", NULL, NULL);
    if (mainwindow == NULL) {
