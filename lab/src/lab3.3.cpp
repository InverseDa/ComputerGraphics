...
void init() {
    ...
    // 读取着色器并使用
    vshader = "lab/labShaders/lab3.3/vshader.glsl";
    fshader = "lab/labShaders/lab3.3/fshader.glsl";
    ...
    mesh->setShininess(27.8974f); //高光系数
...
void mainWindow_key_callback(GLFWwindow *window, int key, int scancode, int action, int mode) {
    ...
    if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) {
        ...
        tmp = specular.x;
        specular.x = std::min(tmp + 0.1, 0.0);
        ...
        specular = mesh->getSpecular();
        tmp = specular.y;
        specular.y = std::min(tmp + 0.1, 0.0);
        mesh->setSpecular(specular);
        ...
        specular = mesh->getSpecular();
        tmp = specular.z;
        specular.z = std::min(tmp + 0.1, 0.0);
        mesh->setSpecular(specular);
...
int main(int argc, char **argv) {
    ...
    GLFWwindow *mainwindow = glfwCreateWindow(600, 600, "Phong-Vertex", NULL, NULL);
    if (mainwindow == NULL) {
