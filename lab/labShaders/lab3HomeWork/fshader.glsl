...
uniform bool ambientOpen;
uniform bool DiffuseOpen;
uniform bool SpecularOpen;
...
void main()
{
    if (isShadow == 1) {
        ...
        // 合并三个分量的颜色，修正透明度
        if (ambientOpen) fColor = I_a;
        else if (DiffuseOpen) fColor = I_d;
        else if (SpecularOpen) fColor = I_s;
        else fColor = I_a + I_d + I_s;
        fColor.a = 1.0;
